...
export function runEngineSelfTest():SelfTestResult{
  ...
  try{
    ...
    add('Respuesta directa no vacía',result.directAnswer.trim().length>20,result.directAnswer.slice(0,100));
    ...
    add('Conclusión disponible',result.conclusion.trim().length>90,result.conclusion.slice(0,100));
    ...
    const candidates=cardImageCandidates('RWS_P_07');
    add('7 de Oros tiene imagen de respaldo',candidates.length>=2&&candidates.some(x=>x.endsWith('/p7.jpg')),candidates.join(' → '));
  }catch(error){
