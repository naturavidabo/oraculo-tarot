...
export function LibraryView() {
  ...
  if (active) return <section className="page">
    ...
    <div className="detail-card">
      <div className="tarot-placeholder large"><span>{active.number ?? '✦'}</span></div>
      ...
      <div className="vector-list">{Object.entries(active.vectors)
        .sort((a,b)=>Math.abs(b[1])-Math.abs(a[1]))
        .slice(0,12)
        .map(([k,v]) => <div key={k}><span>{dimensionLabels[k as TarotDimension] ?? k}</span><b>{v}</b></div>)}</div>
  ...
  return <section className="page">
    <div className="section-title"><div><span className="eyebrow">BIBLIOTECA · CONTENT 1.0</span><h1>78 cartas</h1></div><span>{cards.length}</span></div>
    ...
    <div className="filter-row">
      {([['all','Todos'],['major','Mayores'],['wands','Bastos'],['cups','Copas'],['swords','Espadas'],['pentacles','Oros']] as const).map(([id,label]) =>
        <button key={id} className={suit===id ? 'selected':''} onClick={()=>setSuit(id)}>{label}</button>)}
    </div>
