...
if(pkg.version!=='0.6.0'){failed=true;console.error(`✗ package.json no es 0.6.0: ${pkg.version}`)}
const home=fs.readFileSync('src/features/home/HomeView.tsx','utf8');
if(!home.includes('0.6.0')){failed=true;console.error('✗ HomeView no muestra 0.6.0')}
const workflow=fs.readFileSync('.github/workflows/deploy-pages.yml','utf8');
for(const marker of ['node-version: 22','npm install --no-audit --no-fund','npm run test:core','npm run build','actions/configure-pages@v5','actions/deploy-pages@v4']){
...
if(!failed) console.log('✓ Auditoría de release 0.6.0 superada');
