...
const required=[
  ...
  'scripts/validate-beta-features.mjs','scripts/validate-camera-v4.mjs','scripts/validate-camera-v5.mjs','scripts/validate-camera-v6.mjs','scripts/validate-camera-v7.mjs','scripts/smoke-selector.cjs','scripts/smoke-engine.cjs','scripts/validate-image-manifest.mjs','docs/VALIDATION-1.0-BETA4.md','docs/VALIDATION-1.0-BETA5.md','docs/VALIDATION-1.0-BETA6.md','docs/VALIDATION-1.0-BETA7.md'
];
let failed=false;
for(const file of required){if(!fs.existsSync(file)){failed=true;console.error(`✗ Falta archivo esencial: ${file}`)}}
const pkg=JSON.parse(fs.readFileSync('package.json','utf8'));
if(pkg.version!=='1.0.0-beta.7'){failed=true;console.error(`✗ package.json no es 1.0.0-beta.7: ${pkg.version}`)}
...
const backup=fs.readFileSync('src/db/backup.ts','utf8');if(!backup.includes("appVersion:'1.0.0-beta.7'")){failed=true;console.error('✗ Backup no registra 1.0.0-beta.7')}
