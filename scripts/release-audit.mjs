...
const required=[
  ...
  'src/engine/presentationLabels.ts','src/engine/imageDiagnostics.ts','src/engine/selfTest.ts','src/features/camera/CameraView.tsx',
  ...
  'scripts/smoke-selector.cjs','scripts/smoke-engine.cjs','scripts/validate-image-manifest.mjs','docs/VALIDATION-0.9.1.md'
];
let failed=false;
for(const file of required){if(!fs.existsSync(file)){failed=true;console.error(`✗ Falta archivo esencial: ${file}`)}}
const pkg=JSON.parse(fs.readFileSync('package.json','utf8'));
if(pkg.version!=='0.9.1'){failed=true;console.error(`✗ package.json no es 0.9.1: ${pkg.version}`)}
...
const home=fs.readFileSync('src/features/home/HomeView.tsx','utf8');if(!home.includes('0.9.1')){failed=true;console.error('✗ HomeView no muestra 0.9.1')}
const images=fs.readFileSync('src/data/cardImages.ts','utf8');
if(!images.includes('cardImagePath(cardId),cardImageRemotePath')){failed=true;console.error('✗ La ruta local no es la primera fuente visual')}
...
if(!failed) console.log('✓ Auditoría de release 0.9.1 superada');
